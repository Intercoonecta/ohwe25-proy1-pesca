...
```{r}
pic <- read.csv("C:/Users/jeuee/Documents/1.Cursos/7.Hackaton2025/PublicPSSharkSetType.csv")
```
...
```{r}
copdata <- read.cmems("C:/Users/jeuee/Documents/1.Cursos/7.Hackaton2025/cmems_mod_glo_phy_my_0.083deg_P1M-m_1764094649118.nc")
...
```{r}
thetao <- climatology(copdata$thetao) 
...
```{r}
thetao_1 <- pic |> 
  mutate(latgg = round(lat, 0), longg = round(lon, 0)) |> 
  group_by(year, latgg, longg) |> 
  summarise(n = n())
